...
import { OCCULT_HORRORS_DEFINITIONS, createOccultHorrorsDefinitions } from "../scripts/content.js";
...
function allStages() { return OCCULT_HORRORS_DEFINITIONS.flatMap((definition) => definition.stages); }

test("0.1.0 ships 32 original occult-horror afflictions", () => {
  assert.equal(OCCULT_HORRORS_DEFINITIONS.length, 32);
  const types = new Set(OCCULT_HORRORS_DEFINITIONS.map((definition) => definition.afflictionType));
  assert.deepEqual(types, new Set(["curse", "poison", "disease"]));
  assert.ok(OCCULT_HORRORS_DEFINITIONS.every((definition) => definition.delivery?.injuryPoison === false));
});

test("every definition validates against Affliction Forge schema v2", () => {
  const effectValidator = () => ({ valid: true, issues: [] });
  for (const source of OCCULT_HORRORS_DEFINITIONS) {
...
test("all affliction and stage identities are unique", () => {
  const ids = new Set();
  for (const definition of OCCULT_HORRORS_DEFINITIONS) {
...
test("all entries use the level-based DC baseline", () => {
  for (const definition of OCCULT_HORRORS_DEFINITIONS) assert.equal(definition.checks[0].dc, dcByLevel.get(definition.level), `${definition.id} has unexpected DC`);
});

test("semantic tags are canonical and focused on occult Creature Forge matching", () => {
  const creatures = new Set(); const families = new Set(); const habitats = new Set(); const deliveries = new Set(); const themes = new Set();
  let occultOrigins = 0;
  for (const definition of OCCULT_HORRORS_DEFINITIONS) {
    ...
    for (const tag of definition.themes) {
      ...
      if (parsed.namespace === "creature") creatures.add(parsed.value); if (parsed.namespace === "family") families.add(parsed.value); if (parsed.namespace === "habitat") habitats.add(parsed.value); if (parsed.namespace === "delivery") deliveries.add(parsed.value); if (parsed.namespace === "theme") themes.add(parsed.value); if (tag === "origin:occult") occultOrigins++;
    }
  ...
  assert.ok(families.size >= 6, `Expected broad family coverage, got ${families.size}`);
  assert.ok(habitats.size >= 7, `Expected broad habitat coverage, got ${habitats.size}`);
  assert.ok(deliveries.size >= 8, `Expected broad delivery coverage, got ${deliveries.size}`);
  assert.ok(occultOrigins >= 26, `Expected occult origin on most entries, got ${occultOrigins}`);
  for (const theme of ["dream","mental","shadow","parasite","mutation","corruption","curse","disease","poison"]) assert.ok(themes.has(theme), `Missing theme:${theme}`);
});

test("strange natural creature poisons do not opt into weapon injury-poison charges", () => {
  const poisons = OCCULT_HORRORS_DEFINITIONS.filter((definition) => definition.afflictionType === "poison");
  assert.equal(poisons.length, 8);
...
test("library exercises advanced occult-horror mechanics without requiring a new runtime", () => {
  ...
  assert.ok(OCCULT_HORRORS_DEFINITIONS.some((definition) => definition.progression?.virulent), "Expected virulent afflictions");
  assert.ok(OCCULT_HORRORS_DEFINITIONS.some((definition) => definition.defaultStageCheck?.outcomes?.success?.action === "stay"), "Expected stubborn progression");
...
test("all i18n content tokens resolve in German and English", () => {
  const tokens = [...new Set(OCCULT_HORRORS_DEFINITIONS.flatMap((definition) => collectI18nTokens(definition)))]; assert.ok(tokens.length > 180, "Expected a substantial localized content set");
...
test("definition factory returns independent clones", () => { const a = createOccultHorrorsDefinitions(); const b = createOccultHorrorsDefinitions(); assert.notEqual(a[0], b[0]); a[0].name = "mutated"; assert.notEqual(b[0].name, "mutated"); });

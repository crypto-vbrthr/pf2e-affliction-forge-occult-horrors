import {
  OCCULT_HORRORS_CONTENT_VERSION,
  OCCULT_HORRORS_DEFINITIONS,
  OCCULT_HORRORS_MODULE_ID
} from "./content.js";

const MODULE_ID = OCCULT_HORRORS_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "occult-horrors";
const LIBRARY_ID = "occult-horrors.core";
const PACK_NAME = "affliction-forge-occult-horrors";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_OH.Module.Library", "Occult Horrors"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: OCCULT_HORRORS_CONTENT_VERSION,
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_OH.Module.Title", "PF2E Affliction Forge: Occult Horrors"),
    moduleId: MODULE_ID,
    version: OCCULT_HORRORS_CONTENT_VERSION,
    metadata: {
      themes: ["occult", "horror", "dream", "shadow"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_OH.Module.LibraryDescription", "Original occult-horror afflictions with semantic Creature Forge tags."),
      metadata: {
        themes: ["occult", "horror", "dream", "shadow"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: OCCULT_HORRORS_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: OCCULT_HORRORS_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_OH.Module.SyncFailed", "Occult Horrors could not synchronize its library. See console.")
    );
  });
});
